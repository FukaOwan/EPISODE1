...
//	商品を カテゴリ別 ＋ 売れ筋順 で検索（追記：春山）
	@Query("SELECT i FROM Item i INNER JOIN i.category c INNER JOIN i.orderItemList o WHERE i.deleteFlag =:deleteFlag AND c.id = :categoryId ORDER BY o.quantity DESC,i.id DESC")
